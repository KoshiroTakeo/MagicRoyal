...
public class Patrol : EnemyState                                                                //EnemyStateの機能を継承(スーパークラス)し、EnemyStateでやれることをできるようにする
{
    public Patrol(GameObject _npc, Animator _anim, Transform _player, CharacterData _enemyData)
        : base(_npc, _anim, _player, _enemyData)                                              //baseで継承元(EnemyState)に実際にアクセスする
    {
        name = STATE.PATROL;                                                                //待機状態へ変更
    }
    ...
    public override void Updata()                                                         //Updataメソッドの内容を利用して出力
    {
        ...
        if (Random.Range(0, 10000) < 10) NextState(new Idles(npc, anim, player, enemyData));
    }
